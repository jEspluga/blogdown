...
ggplot(machine, aes(x = timestamp, y=temp))+
  ...
  theme_minimal()+
  labs(title = "Registre Temperatura. (5s.)", x=" ", y="Temperatura (ºC)")+
...
# eigenvalue: valors superiors a 1, variança percentual acumulada
get_eigenvalue(acp)

# gràfic variança acumulada
...
k_mean %>% 
  tidy() %>% 
  select(cluster, size) %>% 
  kable(caption = "Distribució clusters") %>% 
...
#assignació centres de clusters
...
### Distribució horaria de les anomalies
